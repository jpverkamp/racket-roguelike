...
(define (get-tile x y)
  ...
  (unless (hash-has-key? current-level (pt x y))
    ; Get the new tile
    (define new-tile ((level-definition-tile-gen (hash-ref current-level 'gen)) x y))
    ...
    (when (thing-get new-tile 'walkable)
      ...
      (define new-npc ((level-definition-npc-gen (hash-ref current-level 'gen)) x y))
      ...
      (when (null? (thing-get new-tile 'items '()))
        (define new-item ((level-definition-item-gen (hash-ref current-level 'gen)) x y))
...
(define (nothing x y) #f)

; Rats. All of the rats.
(define (rats-only x y)
